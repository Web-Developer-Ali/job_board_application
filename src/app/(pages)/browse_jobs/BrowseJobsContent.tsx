...
export default function BrowseJobs() {
  ...
  const fetchJobs = useCallback(async (page: number) => {
    ...
    setIsLoading(true)
    try {
      const url = searchQuery ? '/api/browse_jobs/serch_query_onjobs' : '/api/browse_jobs/getfilter_jobs'
      const params = {
        salary,
        type,
        location,
  ...
  const handleSearch = useCallback(async (query: string, jobTitle?: string) => {
    ...
    try {
      ...
      const response = await axios.get(`/api/browse_jobs/serch_query_onjobs?${searchParams}`)
  ...
  const handleFilterChange = useCallback((newSalary: string, newType: string, newLocation: string) => {
    setSalary(newSalary)
    setType(newType)
  ...
  const memoizedFilterForm = useMemo(() => 
    filters && (
      <FilterForm
        filters={filters}
        location={location}
        setLocation={(newLocation) => handleFilterChange(salary, type, newLocation)}
        salary={salary}
        setSalary={(newSalary) => handleFilterChange(newSalary, type, location)}
        type={type}
  ...
  const memoizedSearchForm = useMemo(() => <SearchForm onSearch={handleSearch} />, [handleSearch])
          ...
          <PaginationContent>
            <PaginationItem>
              <PaginationPrevious
                ...
                className={`${currentPage === 1 ? "cursor-not-allowed opacity-50" : ""} transition-all duration-300 ease-in-out`}
              >
                Previous
              </PaginationPrevious>
            </PaginationItem>
    ...
    <div className="min-w-full min-h-screen container mx-auto px-4 py-8 dark:bg-gray-900 dark:text-gray-200">
      <div className="flex flex-col md:flex-row justify-between items-center mb-6">
        <h1 className="text-3xl font-bold mb-4 md:mb-0 dark:text-gray-100">
          Browse <span className="text-purple-500 dark:text-purple-400">Jobs</span>
        </h1>
